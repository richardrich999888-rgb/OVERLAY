...
| **MIG-2** | PQC→kTLS data plane: userspace record relay caps throughput; needs kernel TLS transport | Med → **Med (partial — throughput BLOCKED)** | **Migration Platform Phase 2: PQC session-secret extraction, TLS_TX/TLS_RX install path, kTLS capability detection (TCP_ULP=tls probe, incl. stage-aware EINVAL), and fail-safe handling — no ULP ⇒ KtlsUnavailable ⇒ caller FAILS CLOSED (no plaintext userspace-relay fallback) [implemented]+[tested]. Throughput/CPU/latency comparison BLOCKED here: this kernel has no TLS ULP (tls absent from /proc tcp ulp), so kTLS cannot be activated. Recommended target on a ULP host: ≥28 % line / ~2× the 12.8–15.5 % baseline** | `docs/KTLS_DATA_PLANE.md`; `src/kernel_native.rs`; `tests/ktls_*` |
